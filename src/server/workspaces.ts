...
import { AGENT_TARGETS, type AgentId, getTarget, isTargetInstalled, pickPath } from "./agentRegistry";

export type AgentMode = "global" | "local";
export type AgentStatus = "global" | "local" | "missing" | "not_installed" | "drift";

type Agent = { id: AgentId; label: string; filePath: string };
...
const AGENTS: Agent[] = AGENT_TARGETS.map((target) => ({
  id: target.id,
  label: target.label,
  filePath: pickPath(target.rulesFiles)
}));
...
async function classifyAgent(agent: Agent): Promise<AgentStatus> {
  if (!isTargetInstalled(getTarget(agent.id))) return "not_installed";
  ...
  const agents = await Promise.all(
    AGENTS.map(async (agent) => {
      ...
      return {
        ...
        label: agent.label,
        path: agent.filePath,
        installed: isTargetInstalled(getTarget(agent.id)),
        status,
        mode,
...
export async function setAgentMode(agentId: AgentId, mode: AgentMode): Promise<{ ok: true }> {
  ...
  if (!isTargetInstalled(getTarget(agent.id))) throw new Error("Agent is not installed");
